...
`saturn.run()` creates a window with the OpenGL renderer by default and passes a [`Page`](./api/Page.md) to `main`. Add controls with `page.add()`. After changing a control, call `control.update()` or `page.update()` to request a redraw. In a callback, `event.control` refers to the control that triggered the event.
